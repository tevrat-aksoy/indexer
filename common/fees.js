...
export function decodeTransfersInBlock({ header, events }) {
    ...
    return events.map(({ event, transaction }) => {
        ...
        const [sender, amount0Low, amount0High, amount1Low, amount1High] = event.data;

        const amount0Raw = uint256.uint256ToBN({ low: amount0Low, high: amount0High });
        const amount1Raw = uint256.uint256ToBN({ low: amount1Low, high: amount1High });
