...
async function fetchGraphQLData() {
  ...
  let startBlock = 602407;
  let endBlock = 623987;
  let i = 0;
  for (let block = startBlock; block <= endBlock; block += 200) {
    ...
    try {
      const response = await axios.post("http://localhost:8080/v1beta1/relay", {
        query: `
        query {
          pairs_connection(where: {block_number: {_lte: ${block}}}){
            edges {
              node {
                pair
                amount
  ...
  let filePath = './result/multipleBlocks.json'
